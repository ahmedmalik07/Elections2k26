...
import { runnerScore } from "./runner.mjs";
...
export function validateScore(b, elapsed, mode = "classic") {
  ...
  if (mode === "dash") {
    for (const k of ["score", "votes", "distance", "durationMs"])
      if (!Number.isSafeInteger(b[k]) || b[k] < 0)
        throw Error("Invalid run. Dobara khelo.");
    // Top speed is 34 m/s and a pickup row spawns at most every 0.85 s.
    if (
      elapsed > 1800000 ||
      b.durationMs < 1000 ||
      b.durationMs > elapsed + 1500 ||
      b.distance > (b.durationMs / 1000) * 34 + 5 ||
      b.votes > b.durationMs / 850 + 3 ||
      b.score !== runnerScore(b.distance, b.votes)
    )
