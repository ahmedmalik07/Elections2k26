...
export function validateScore(b, elapsed, mode = "classic") {
  ...
  if (mode === "dash") {
    ...
    if (
      // Campus Dash is deliberately an endurance game. Scores have no fixed
      // ceiling: their valid maximum grows with actual elapsed play time.
      // A run can play continuously for a full day and still be submitted or
      // retried for up to a month if the connection dropped at the finish.
      elapsed > 31 * 86400000 ||
      b.durationMs > 24 * 3600000 ||
      b.durationMs < 1000 ||
