...
// The full leaderboard keeps a generous history while the compact cards still
// request only their top 5 or 10. 250 rows fits comfortably in one Firestore
// snapshot document and one Redis sorted set.
...
// Redis and Firestore's boards/<mode> repair doc are both written on every
// score, independently, and can briefly disagree (a race between two
// simultaneous submissions, a slow write, a dropped request). Whichever one a
// reader catches out of date must never make a player's best score disappear
// from what everyone sees: this keeps the higher score for every id seen in
// either list, rather than picking one whole list over the other.
export function mergeBoards(a, b) {
  const best = new Map();
  for (const row of [...(a || []), ...(b || [])]) {
    if (!row?.id) continue;
    const known = best.get(row.id);
    if (!known || (row.bestScore || 0) > (known.bestScore || 0))
      best.set(row.id, row);
  }
  return [...best.values()]
    .filter((r) => (r.bestScore || 0) > 0)
...
// `redis` is injected so the tests can run without a network, and so the whole
// file works unchanged when no Upstash credentials are configured.
/**
 * @param {{ redis?: {
 *   ready: () => boolean,
 *   board: (mode: string, limit?: number) => Promise<any[] | null>,
 *   replace: (mode: string, rows: any[]) => Promise<any>,
 * } | null }} [options]
 */
export function createBoardCache({ redis = null } = {}) {
  ...
  return {
    ...
    async get(store, key, dept, mode, now = Date.now()) {
      ...
      // Redis holds the player boards and never runs out of daily quota, so it
      // is asked first; Firestore remains the record everything is rebuilt from.
      if (redis?.ready?.() && !dept) {
        try {
          const rows = await redis.board(mode, BOARD_LIMIT);
          if (rows?.length) {
            // One snapshot read per cache interval repairs a missed Redis
            // write by merging it back in. Neither source is ever discarded
            // wholesale: that previously let a stale Firestore snapshot hide
            // a score that Redis already had correctly.
            let saved;
            try {
              saved = (
                await withDeadline(store.doc(`boards/${key}`).get())
              ).data();
            } catch {}
            const fresh = {
              // The true "last score saved" time only Firestore's transaction
              // records; a fresh Redis-only read is not itself a new save.
              at: saved?.at > 0 ? saved.at : now,
              rows: saved?.rows?.length ? mergeBoards(saved.rows, rows) : rows,
              checkedAt: now,
              source: saved?.rows?.length ? "redis+firestore" : "redis",
            };
      ...
      try {
        ...
        // Seed Redis from the rebuilt board, so it can serve the next requests.
        if (redis?.ready?.() && !dept)
          await redis.replace(mode, built.rows).catch(() => {});
