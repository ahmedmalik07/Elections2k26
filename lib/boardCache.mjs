...
export const BOARD_SNAPSHOT_MS = 90000; // age at which a snapshot is rebuilt
  ...
  return {
    ...
    async get(store, key, dept, mode, now = Date.now()) {
      ...
      try {
        ...
        if (saved?.rows && now - saved.at < BOARD_SNAPSHOT_MS) {
        ...
        const fallback = saved?.rows ? saved : memo;
        if (!fallback) throw e;
        const stale = {
          at: fallback.at,
          rows: fallback.rows,
