    ...
    async get(store, key, dept, mode, now = Date.now()) {
      ...
      if (redis?.ready?.() && !dept) {
        try {
          ...
          if (rows?.length) {
            // One snapshot read per cache interval repairs missed Redis writes.
            // Previously a non-empty Redis board bypassed Firestore forever.
            let saved;
            try {
              saved = (
            ...
            if (
              saved?.at > 0 &&
              saved.rows?.length &&
              JSON.stringify(saved.rows) !== JSON.stringify(rows)
            ) {
              const fresh = {
                at: saved.at,
                rows: saved.rows,
              ...
              // Do not DEL/reseed here: that can erase a concurrently saved best.
              return fresh;
            }
            if (!saved || saved.at > 0) {
              const fresh = {
